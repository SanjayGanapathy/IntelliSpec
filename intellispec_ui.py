...
class ChatWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # API Key field (only shown if no key in environment)
...
class SpectrophotometerUI(QMainWindow):
    """
    Main UI class for the Spectrophotometer application.
    Handles serial communication with the device and displays measurements.
    """
    def __init__(self):
        super().__init__()
        ...
        self.dark_voltage = 0.02
        self.initial_voltage = None  # Set during calibration
    ...
    def init_ui(self):
        ...
        # Create splitter for main content and chat sidebar
    ...
    def handle_serial_data(self, line):
        ...
        Args:
            line (str): Data received from serial port
        """
        ...
        try:
            ...
            if "Initial Voltage (Blank):" in line:
                print("Processing initial voltage...")
                voltage_str = line.split(":")[-1].strip()
                print(f"Extracted voltage string: '{voltage_str}'")
                try:
                    voltage = float(voltage_str)
                    self.initial_voltage = voltage
                    print(f"Set initial voltage to: {self.initial_voltage}")
                    self.voltage_value.setText(f"{voltage:.3f} V")
                    # Since this is calibration, set absorbance to 0 and transmittance to 100%
            ...
            # Handle regular voltage reading
                ...
                print("Processing voltage reading...")
                voltage_str = line.split(":")[-1].strip()
                print(f"Extracted voltage string: '{voltage_str}'")
                try:
                    voltage = float(voltage_str)
                    ...
                    if self.initial_voltage is not None:
                        print(f"Calculating with initial_voltage: {self.initial_voltage}")
                        # Calculate absorbance using Beer-Lambert equation
                        enters = self.initial_voltage - self.dark_voltage
                        passes = voltage - self.dark_voltage
                        print(f"enters: {enters}, passes: {passes}")
                        
                        try:
                            if enters <= 0 or passes <= 0:
                                print("enters or passes <= 0, setting absorbance to 2.00")
                                absorbance = 2.00
                            elif enters/passes > 100:
                                print("enters/passes > 100, setting absorbance to 2.00")
                                absorbance = 2.00
                            else:
                                absorbance = math.log10(enters / passes)
                                print(f"Calculated absorbance: {absorbance}")
                                if math.isnan(absorbance) or math.isinf(absorbance):
                                    print("Absorbance is NaN or Inf, setting to 2.00")
                                    absorbance = 2.00
                        except (ValueError, ZeroDivisionError) as e:
                            print(f"Error calculating absorbance: {e}")
                            absorbance = 2.00
                        ...
                        if absorbance >= 2.00:
                            print("Absorbance >= 2.00, setting transmittance to 0.0")
                            transmittance = 0.0
                        else:
                            try:
                                transmittance = 100 * (passes / enters)
                                print(f"Calculated transmittance: {transmittance}")
                                if math.isnan(transmittance) or math.isinf(transmittance):
                                    print("Transmittance is NaN or Inf, setting to 0.0")
                                    transmittance = 0.0
                            except ZeroDivisionError:
                                print("Division by zero in transmittance calculation")
                                transmittance = 0.0
                        
                        self.transmittance_value.setText(f"{transmittance:.1f} %")
                    elif not self.calibrating:
                        print("No initial voltage set - using current reading as reference")
                        self.initial_voltage = voltage
                        print(f"Auto-setting initial voltage to current voltage: {voltage}")
                        # Set baseline readings for no sample
                        self.absorbance_value.setText("0.000 A")
                        self.transmittance_value.setText("100.0 %")
                except ValueError as e:
                    print(f"Error parsing voltage: {e}")
                
            # Handle absorbance reading from Arduino
            elif "Absorbance:" in line and not self.calibrating:
                print("Processing absorbance reading from Arduino...")
                if not line.strip().endswith('nan'):  # Only process if not NaN
                    abs_str = line.split(":")[-1].strip()
                    print(f"Extracted absorbance string: '{abs_str}'")
                    try:
                        absorbance = float(abs_str)
                        if math.isnan(absorbance) or math.isinf(absorbance):
                            print("Absorbance is NaN or Inf from Arduino, setting to 2.00")
                            absorbance = 2.00
                        self.absorbance_value.setText(f"{absorbance:.3f} A")
                        
                        # Update transmittance based on absorbance
                        transmittance = 100 * (10 ** -absorbance)
                        print(f"Calculated transmittance from absorbance: {transmittance}")
                        self.transmittance_value.setText(f"{transmittance:.1f} %")
                    except ValueError as e:
                        print(f"Error parsing absorbance: {e}")
    ...
    def calibrate(self):
        if self.serial_thread:
            ...
            QTimer.singleShot(12000, self.finish_calibration)  # Re-enable after 12 seconds
    ...
    def measure(self):
        if self.serial_thread:
            ...
            QTimer.singleShot(7000, self.enable_buttons)  # Re-enable after 7 seconds
